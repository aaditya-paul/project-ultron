    ...
    try:
        ...
        if not findings:
            ...
            for idx, finding in enumerate(findings, start=1):
                ...
                loc_str = f"{file_name}:{line_no}" if file_name and line_no else file_name
                ...
                if loc_str:
                    desc_text += f"<br/><b>Location:</b> {loc_str}"
                if source and sink:
                    desc_text += f"<br/><b>Flow:</b> {source} &rarr; {sink}"
                if path:
                    # Limit path nodes and ensure space separation so ReportLab can word-wrap cleanly
                    clean_nodes = [str(p).strip() for p in path]
                    if len(clean_nodes) > 6:
                        trimmed = clean_nodes[:3] + [f"... ({len(clean_nodes) - 6} nodes) ..."] + clean_nodes[-3:]
                    else:
                        trimmed = clean_nodes
                    path_str = " &rarr; ".join(trimmed)
                    if len(path_str) > 250:
                        path_str = path_str[:247] + "..."
