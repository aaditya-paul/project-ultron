...
from ir import IRVar, IRAccess, IRCallExpr, IRLiteral, IRCall, IRAssign, IRBranch, IRReturn, IRFunction, IRExpr
...
def _index_expr(expr, index, file_path, line):
    ...
    if isinstance(expr, IRAccess):
        ...
        for p in expr.path:
            if isinstance(p, IRExpr):
                _index_expr(p, index, file_path, line)
    elif isinstance(expr, IRCallExpr):
        if expr.receiver:
...
def _index_stmt(stmt, index, file_path, line):
    ...
    if isinstance(stmt, IRCall):
        ...
        # Also index target variables (plain, property access, or destructured)
        target_str = stmt.target.strip()
        if "." in target_str:
...
def get_node_human_label(nid: str, node_index: dict) -> tuple[str, str, int]:
    ...
    label = fallback_map.get(prefix, nid)
    return label, "", 0
...
def build_security_graph_from_ir(ir_modules, call_graph, taint_paths):
    ...
    for flow_id, tp in enumerate(taint_paths):
        ...
        # Resolve path node IDs to human-readable expressions & locations
        resolved_path_labels = [tp.source_tag]
        for nid in tp.path_node_ids:
            lbl, fp, line = get_node_human_label(nid, node_index)
            fp_basename = os.path.basename(fp) if fp else ""
            if fp_basename and line:
                resolved_path_labels.append(f"{fp_basename}:{line} ({lbl})")
            else:
                resolved_path_labels.append(f"{lbl}")
        resolved_path_labels.append(tp.sink_target)

        full_path = [source_fnid] + [f"{tp.file_path}::{nid}" for nid in tp.path_node_ids] + [sink_fnid]

        # Extract primary location from sink or source node
        sink_info = node_index.get(tp.sink_node_id, {})
        source_info = node_index.get(tp.source_node_id, {})
        flow_file = tp.file_path or sink_info.get("file_path") or source_info.get("file_path") or ""
        flow_line = sink_info.get("line") or source_info.get("line") or 0
